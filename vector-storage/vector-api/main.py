    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        try:
            ...
            if create_response.status_code not in [200, 201, 409]:
                print(
                    f"[WARN] Collection create for '{name}' returned {create_response.status_code}: {create_response.text}",
                    flush=True,
                )
        ...
        if response.status_code != 200:
            raise HTTPException(
                status_code=500, detail=f"ChromaDB error: {response.text}"
            )
        ...
        if isinstance(data, list):
            ...
            raise HTTPException(
                status_code=500, detail=f"Collection '{name}' not found after creation"
            )
...
async def add_document(
    collection_id: str, doc_id: str, text: str, embeddings: list, metadata: dict
):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        if response.status_code not in [200, 201]:
            raise HTTPException(
                status_code=500, detail=f"ChromaDB add error: {response.text}"
            )
...
async def query_collection(collection_id: str, embeddings: list, top_k: int):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        response = await client.post(
            ...
            json={"query_embeddings": [embeddings], "n_results": top_k},
        )
        if response.status_code != 200:
            raise HTTPException(
                status_code=500, detail=f"ChromaDB query error: {response.text}"
            )
...
async def list_documents(collection_id: str, limit: int, offset: int = 0):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        if response.status_code != 200:
            raise HTTPException(
                status_code=500, detail=f"ChromaDB list error: {response.text}"
            )
        return response.json()
            ...
            matches.append(
                {
                    ...
                    "similarity": 1 / (1 + result["distances"][0][i])
                    if result.get("distances")
                    else None,
                }
            )
...
async def delete_documents(collection_id: str, ids: list[str]):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        response = await client.post(
            f"{CHROMA_API_BASE}/collections/{collection_id}/delete", json={"ids": ids}
        )
        if response.status_code not in [200, 204]:
            raise HTTPException(
                status_code=500, detail=f"ChromaDB delete error: {response.text}"
            )
        return True
...
async def delete_project(
    request: UninitProjectRequest, _: bool = Depends(verify_token)
):
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        if response.status_code == 404:
            ...
            return {
                "status": "success",
                "message": f"Project collection '{collection_name}' not found, nothing to delete",
            }
        ...
        return {
            "status": "success",
            "message": f"Project collection '{collection_name}' deleted",
        }


@app.post("/check/project")
async def check_project(request: CheckProjectRequest, _: bool = Depends(verify_token)):
    """Check if project collection exists (without creating it)"""
    collection_name = f"project_{request.project_id.lower().replace(' ', '-')}"

    async with httpx.AsyncClient(timeout=10.0) as client:
        # List collections to see if ours exists
        response = await client.get(f"{CHROMA_API_BASE}/collections")
        ...
        if isinstance(data, list):
            for collection in data:
                if collection.get("name") == collection_name:
                    ...
                    break
        elif data.get("name") == collection_name:
            exists = True
            collection_id = data["id"]

        if exists and collection_id:
            # Get count if it exists
            count_response = await client.get(
                f"{CHROMA_API_BASE}/collections/{collection_id}/count"
            )
